...
export async function fetchArtworkData(
  http: HttpManager, artworkId: string
): Promise<WildDreamArtworkResponse> {
  const data = await http.getJson<WildDreamArtworkResponse>(
    `/Art/view/${encodeURIComponent(artworkId)}/ajax/1`
  );
...
export async function fetchUserProfileData(
  http: HttpManager, userName: string
): Promise<WildDreamUserProfileResponse> {
  const data = await http.getJson<WildDreamUserProfileResponse>(
    `/Art/userpage/profile/userpagename/${encodeURIComponent(userName)}/ajax/1`
  );
 ...
 * @param size
 */
export function getUserHeadshotURL(
